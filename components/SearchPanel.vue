...
const groupedResults = computed(() => {
  const map = new Map<string, { vaultName: string, results: SearchResult[] }>()
  for (const r of results.value) {
    const group = map.get(r.doc.vaultId)
    if (!group) {
      map.set(r.doc.vaultId, { vaultName: r.doc.vaultName, results: [r] })
    }
    else {
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div
      v-else
      class="flex-1 overflow-auto p-2 space-y-4"
    >
      <div
        v-for="group in groupedResults"
        :key="group.vaultName"
      >
        <div class="text-xs font-semibold text-muted uppercase tracking-wider mb-1 px-1">
          {{ group.vaultName }}
        </div>
        <div class="space-y-1">
          <div
          ...
          >
            <div
              ...
              v-html="highlightSnippets(result.doc.name, searchTerms)"
            />
            <div
